...
V11.3 进一步把任务状态改成用户可理解的状态简报：系统判断、原因和用户动作。
V11.4-V11.6 继续收缩信息层级：右侧只保留任务事实和状态简报，语音按钮成为主入口之一，路线专项验收改成可直达、可复盘的 4/4 稳定页面。
...
- **语音主入口**：语音识别先进入确认区，确认后才触发物理执行，避免 ASR 误识别直接动作。
- **路线与执行可视化**：展示机器人移动、物体跟随、路线 overlay 和执行时间线。
- **执行中变化处理**：支持暂停、改目的地、换对象和异常恢复。
- **核心流程展示**：用四步流程条展示理解、定位、安全门和执行状态。
- **状态简报**：把澄清、确认、接管、恢复和执行中状态转成“系统判断 / 原因 / 用户动作”。
- **核心验收**：默认只展示 10 条主链路用例，证明语音理解、对象澄清、安全确认、执行改口和多轮补全稳定。
- **路线专项复盘**：`test-plan.html?filter=route` 展示 direct、绕行、导航和老人递送 4 条路线用例，目前 4/4 通过。
...
1. 继续强化语音输入到确认区的真实体验，例如识别置信度、文本修正和确认后执行。
2. 在不增加新页面的前提下，把对象澄清、安全确认、路线预览和执行反馈做成更强的主流程证明。
