...
- 左栏隐藏完整语音 PM Demo、执行接管和指代对象折叠项；语音确认、执行接管和指代对象只在需要时展开。
- 试跑场景从五个压缩为四个：日常执行、对象澄清、安全确认、执行改口。
- 右栏修复澄清/确认状态一致性，让“下一步”卡片跟 pending 状态同步。
- 目标是把 Demo 从功能堆叠继续收敛成面试现场可快速理解的产品控制台。

## V11.2: Core Flow Evidence

- 主 Demo 右侧新增“理解 / 定位 / 安全门 / 执行”四步核心流程条。
- 测试计划页默认进入“核心”筛选，只展示 10 条主链路用例。
- 高级复盘沙盒默认折叠，避免默认验收页被事故注入和全量细项抢占。
- 默认核心验收当前 10/10 通过；路线深测保留在全量/路线筛选中，作为后续专项复盘。
- 目标是在继续做减法的同时，把 Voice-to-Action 主闭环证据做得更直观。

## V11.3: Status Language

- 主 Demo 新增状态简报，把状态解释统一为“系统判断 / 原因 / 用户动作”。
- `buildStatusBrief` 成为右侧任务状态和下一步卡片的统一文案来源。
- 澄清、确认、接管、恢复、执行中和完成状态都改成用户可理解表达。
